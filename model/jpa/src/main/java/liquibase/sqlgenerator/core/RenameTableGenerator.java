...
import org.keycloak.connections.jpa.updater.liquibase.NuoDBDatabase;
...
public class RenameTableGenerator extends AbstractSqlGenerator<RenameTableStatement> {
    ...
    public Sql[] generateSql(RenameTableStatement statement, Database database, SqlGeneratorChain sqlGeneratorChain) {
        ...
        } else if (database instanceof NuoDBDatabase) {
            sql = "ALTER TABLE " + database.escapeTableName(statement.getCatalogName(), statement.getSchemaName(), statement.getOldTableName()) + " RENAME TO " + database.escapeTableName(statement.getCatalogName(), statement.getSchemaName(), statement.getNewTableName());
